...
func ValidatePlatform(p *openstack.Platform, n *types.Networking, fldPath *field.Path, fetcher ValidValuesFetcher, c *types.InstallConfig) field.ErrorList {
	...
	machineMask := n.MachineCIDR.Mask
	if p.AciNetExt.NeutronCIDR.String() != "" {
                neutronMask := p.AciNetExt.NeutronCIDR.Mask
                if machineMask.String() != neutronMask.String() {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("neutronCIDR"), p.AciNetExt.NeutronCIDR.String(), "The CIDRs specified in the machineCIDR (" + n.MachineCIDR.String() + ") and neutronCIDR (" + p.AciNetExt.NeutronCIDR.String() + ") configurations in the install-config.yaml have different subnet masks (machineCIDR mask: " + machineMask.String() + ", neutronCIDR mask: " + neutronMask.String()))
                }
        } else {
