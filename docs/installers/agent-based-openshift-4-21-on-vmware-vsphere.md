...
# Installing Agent Based OpenShift 4.21 on VMware vSphere

## Table of contents

* [Agent Based OpenShift 4.21 on VMware vSphere](#agent-based-openshift-4.21-on-vmware-vsphere)

* [Prerequisites for Installing Agent Based OpenShift 4.21 on VMware vSphere](#prerequisites-for-installing-agent-based-openshift-4.21-on-vmware-vsphere)

* [Installing Agent Based OpenShift 4.21 on VMware vSphere](#installing-agent-based-openshift-4.21-on-vmware-vsphere)
...
## Agent Based OpenShift 4.21 on VMware vSphere

Cisco ACI supports Agent Based Red Hat OpenShift 4.21 on VMware vSphere. This document provides the instructions to provision OpenShift 4.21
on VMware vSphere with the Container Network Interface (CNI) plug-in.


## Prerequisites for Installing Agent Based OpenShift 4.21 on VMware vSphere

To install Agent Based OpenShift Container Platform (OCP) 4.21 on VMware vSphere, fulfill the following prerequisites:

### Cisco ACI

* Download the acc-provision tool version `6.1.1.7` or later.
* Specify the `--flavor` option value as `openshift-4.21-agent-based-esx` and use the `-z` option. 
* The tool creates a `.tar` archive file as specified by the `-z` option value. You need this archive file during installation. Make sure that the Cisco ACI container images that are specified as input to the acc-provision tool are version `6.1.1.7` or later.
...
Complete the tasks in the previous section: Prerequisites for Installing Agent Based OpenShift 4.21 on VMware vSphere .
...
$ ~/openupi$ pwd
/home/<user>/openupi
$ ~/openupi$ acc-provision -a -c acc_provision_input.yaml  -f openshift-4.21-agent-based-esx -u <user> -p <password> -o aci_deployment.yaml -z aci deployment.yaml.tar.gz
```
...
This section contains sample files that you need for installing agent based OpenShift 4.21 on Vmware vSphere.
...
* Follow steps to create machineset as mentioned in the link: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html/machine_management/managing-compute-machines-with-the-machine-api#creating-machineset-vsphere to scale the cluster.
...
```acc-provision -d -f openshift-4.21-agent-based-esx -c acc-input-file -u user -p password```

Example:
`acc-provision -d -f openshift-4.21-agent-based-esx -c acc-input-config.yaml -u admin -p password`
