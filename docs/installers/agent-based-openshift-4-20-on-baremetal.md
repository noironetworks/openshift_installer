...
# Installing Agent Based OpenShift 4.20 on a Bare Metal Server

## Table of contents

* [Agent-Based OpenShift 4.20 on Bare Metal](#agent-based-openshift-4.20-on-bare-metal)
* [Requirements for supporting Agent Based OpenShift 4.20 on a Bare Metal Server](#requirements-for-supporting-agent-based-openshift-4.20-on-a-bare-metal-server)
...
## Agent-Based OpenShift 4.20 on Bare Metal
...
the OpenShift 4.20 container platform documentation.*

**Note** This document can not be used standalone. This document
should be used along with the *Red Hat OpenShift 4.20 Installing an
on-premise cluster with the Agent-based Installer* document to perform
the OpenShift cluster installation.

## Requirements for supporting Agent Based OpenShift 4.20 on a Bare Metal Server
...
1.  For details of the location from where you can download the
    installer, see the OpenShift 4.20 document titled, *Installing an
...
$ ~/openupi$ pwd
/home/<user>/openupi
$ ~/openupi$ acc-provision -a -c acc_provision_input.yaml -f openshift-4.20-agent-based-baremetal -u <user> -p <password> -o aci_deployment.yaml -z aci deployment.yaml.tar.gz
```
...
cluster; see the *Redhat OpenShift 4.20 document* (mentioned earlier in
the chapter).

## Known Caveats
* Assisted installer cluster installation fails with IP collision validation, due to a Proxy ARP request
